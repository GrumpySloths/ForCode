...
class GymEnv_RL(gym.Env):
    ...
    def reset(self, **kwargs):
        ...
        obs, info = self.agent.reset(next_ETG_act=self.last_ETG_act,
    ...
    def Reward(self, info):
        ...
        re_vel_foot = 1 - self.c_prec(min(info["vel_foot"], VEL_D_FOOT),
                                      VEL_D_FOOT, 0.1)  #奖励小鼠foot处于想要的速度
        re_yaw = 1 - self.c_prec(info["euler_z"], 0, 0.5)  #奖励小鼠处于想要的yaw角度
        self.debug("vel_body={},yaw={},vel_foot={}".format(
            info["vel_body"], info["euler"][2], info["vel_foot"]))
