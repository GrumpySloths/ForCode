...
TERRAIN_CMAP = 'Greens'
DEFAULT_PATH = '/home/niujh/ForCode/NewMujo_test/src/terrain'
STEP = 0.1
...
def generate_hills(width, height, nhills):
    ...
    # generate hilltops
    xm, ym = np.mgrid[xmin:xmax:width / np.sqrt(nhills),
                      ymin:ymax:height / np.sqrt(nhills)]
...
def save_heightfield(x, y, hfield, fname, path=None):
    ...
    plt.figure()
    plt.contourf(
        x, y, -hfield, 100, cmap=TERRAIN_CMAP
    )  # terrain_cmap is necessary to make sure tops get light color
...
if __name__ == '__main__':
    ...
    height = 20
    nhills = 1000
    x, y, hfield = generate_hills(width, height, nhills)
    # print(hfield)
    save_heightfield(x, y, hfield, fname="terrain_map_1000.png")
    print("save success")
